...
class HATemperatureSensor(HomeAssistantSensorDevice):
    def __init__(self):
        ...
        self.last_refresh_time = 0  # 记录上次LCD刷新时间
        self.last_temperature = None  # 缓存最新温度值
        self.init()
    ...
    def do_work(self):
        while True:
            current_time = self.time_util.get_current_timestamp() # 当前时间（秒）
            print("current_time:", current_time)
            
            device, temperature = self.ds18b20_temperature_sensor.collect_temperature_result()
            print("temperature:", temperature)
            self.last_temperature = temperature  # 更新缓存温度
            
            
            # 每5分钟发送一次MQTT数据
            if current_time - self.last_send_time >= TEMPERATURE_SEND_MSG_FREQ:
                self.do_mqtt_publish_device_msg(str(temperature))
                self.last_send_time = current_time  # 更新发送时间
            
            # 每1秒刷新LCD
            if current_time - self.last_refresh_time >= LCD_REFRESH_FREQ:
                self.esp32160lcd.clear_msg()
                # 显示时间和温度
                self.esp32160lcd.show_msg(
                    self.time_util.get_current_datetime_hms(),
                    #current_time - self.last_send_time,
                    "temp: " + str(self.last_temperature)
                )
                self.last_refresh_time = current_time  # 更新刷新时间
                
            #print("last_send_time:", self.last_send_time)
            #print("last_refresh_time:", self.last_refresh_time)
            #self.do_mqtt_publish_device_msg(str(temperature))
            #self.esp32160lcd.show_msg(self.time_util.get_current_datetime_hms(),"hello")
            #self.esp32160lcd.show_msg(self.time_util.get_current_datetime_hms(), "temp: " + str(temperature))
